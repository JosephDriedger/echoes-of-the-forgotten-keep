...
namespace game
{
    void MovementSystem::Update(engine::Registry& registry, engine::Entity playerEntity, const engine::Input& input, float deltaTime)
    {
        ...
        if (registry.HasComponent<CombatState>(playerEntity))
        {
            ...
            {
                if (registry.HasComponent<AnimationState>(playerEntity))
                    registry.GetComponent<AnimationState>(playerEntity).IsMoving = false;
                return;
            }
        }

        auto& transform = registry.GetComponent<Transform>(playerEntity);
        ...
        bool isAttacking = registry.HasComponent<CombatState>(playerEntity) &&
               registry.GetComponent<CombatState>(playerEntity).IsAttacking;

        if (registry.HasComponent<AnimationState>(playerEntity))
        {
            registry.GetComponent<AnimationState>(playerEntity).IsMoving = isMoving && !isAttacking;
        }
